// project → 生成 clip(take)的本机绑定索引(A 方案)。复用 file-index 的形态:每项目一个 JSON、
...
const index = createFileIndex<ClipRecord>('clips-index', 'clipId');

/** 读回列表;文件不存在 / 解析失败一律回空。 */
export const readClipIndex = index.read;

/** 追加一条(最新在前;clipId 去重)。appendClip 之间由生成队列单 worker 串行。
 *  ponytail: 与走 HTTP DELETE 路径的 removeClip 不共享锁 → 同项目「生成完成」与「删除」在几毫秒
 *  文件 IO 窗口内交错可致索引丢/复活一条(仅索引不一致,无崩溃)。要严格一致再上 per-project 互斥锁。 */
export const appendClip = index.append;
...
export const removeClip = async (projectId: string, clipId: string): Promise<boolean> => {
  ...
  await index.write(projectId, records.filter((r) => r.clipId !== clipId));
