...
// 「Clips」组(agent,>1)走 @dnd-kit 拖拽排序(见 SortableClipsGrid);Resources(creator)与单图静态铺。
export function AssetGrid({ projectId, assets }: { projectId: string; assets: NonNullable<BffProjectDetail['assets']> }) {
  ...
  const keyOf = (a: Asset) => a.contentId ?? a.id
  // 加入编辑器:探测尺寸 → 建素材 + 时间线条目落到右侧编辑器(见 add-to-editor)
  const handleAdd = async (a: Asset) => {
    setAdding(a.id)
    try {
  ...
  // 打开预览灯箱;el(缩略图按钮)仅用于取展开原点 rect。拖拽项由 dnd-kit 的激活阈值区分「拖 vs 点」→ 纯点击才到这。
  const openPreview = (a: Asset, el?: Element | null) =>
    viewer.open(assets.indexOf(a), el ? { currentTarget: el } : undefined)
  // 单张瓦片内容(缩略图 + 加入编辑器 + 删除),creator/agent 两组共用;外层容器(可拖/静态)各自包
  const tileInner = (a: Asset) => (
    <>
      <button
        type="button"
        onClick={(e) => openPreview(a, e.currentTarget)}
      ...
      >
        ...
        <Thumb url={a.thumbnailUrl || a.url} kind={a.thumbnailUrl ? 'image' : a.kind} className="size-full rounded-none" />
      ...
      {groups.map(({ key, label }) => {
        ...
        return (
          <Group key={key} title={`${label} (${list.length})`}>
            {key === 'agent' && list.length > 1 ? (
              // Clips 组(多于一张)→ @dnd-kit 二维网格拖拽排序,落库走 reorder(新 content_id 顺序)
              <SortableClipsGrid
                items={list}
                getKey={keyOf}
                renderTile={tileInner}
                onReorder={(contentIds) => reorder.mutate({ projectId, contentIds })}
              />
            ) : (
